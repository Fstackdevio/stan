<?php
$app->get('/viewResource', function($request, $response){
        $db = new DbHandler();
        $response = array();
...
$app->post('/addResource', function($request, $response){
        $response = array();
        ...
        $isPrefExist = $db->getOneRecord("SELECT 1 from pref_resource WHERE name='$name' or type='$type' or content='$content'");
            if(!$isPrefExist){
                ...
                if($result != null){
                    ...
                    return $this->response->withJson($response)->withStatus(202);
                }
            });

$app->put('/editResource/{id}', function($request, $response, $id){
        $response = array();
        $r = json_decode($request->getBody());
        $id = $id['id'];
...
$app->delete('/deleteResource/{id}', function($request, $response, $id){
        $response = array();
        $r = json_decode($request->getBody());
        $id = $id['id'];
        $condition = array('_id'=>$id);
